...
public class DrawTextPanel extends JPanel  {
	...
	private ArrayList<DrawTextItem> theStringList = new ArrayList<>();  // change to an ArrayList<DrawTextItem> !
	...
	private class Canvas extends JPanel {
		...
		protected void paintComponent(Graphics g) {
			...
			for(DrawTextItem theString : theStringList)
				if(theString != null)
					theString.draw(g);
		}
	}
	...
	public void doMousePress( MouseEvent e ) {
		...
		// if doing the right click, undo the last string
		if(e.getButton() == MouseEvent.BUTTON3){
			doMenuCommand("Remove Item");
		} else {
	...
	//		s.setBackgroundTransparency(0.7);  // Default is 0, meaning background is not transparent.
			...
			theStringList.add(s);  // Set this string as the ONLY string to be drawn on the canvas!
	...
	private void doMenuCommand(String command) {
		if (command.equals("Save...")) { // save all the string info to a file
			try{
				File file = fileChooser.getOutputFile(this, "Select File Name", "textcollage.txt");
				PrintWriter pw = new PrintWriter(file);
				FileWriter fw = new FileWriter(file);
				Color bgColor = canvas.getBackground();
				pw.println(bgColor.getRGB());
				for(DrawTextItem theString : theStringList){
					pw.println(theString.getString());
					pw.println(theString.getX());
					pw.println(theString.getY());
					Color textColor = theString.getTextColor();
					pw.println(textColor.getRGB());
				}
				pw.close();
			} catch (Exception e){
			...
			try {
				 ...
				 Color bgColor = new Color(scanner.nextInt());
				 ...
				 while(scanner.hasNext()){
					 ...
					 DrawTextItem str = new DrawTextItem(text, x, y);
					 Color textColor = new Color(Integer.parseInt(scanner.nextLine()));
					 str.setTextColor(textColor);
			...
			theStringList.clear();   // Remove the ONLY string from the canvas.
			undoMenuItem.setEnabled(false);
			canvas.repaint();
		}
		else if (command.equals("Remove Item")) { // remove the most recently added string
			theStringList.remove(theStringList.size()-1); // Remove the ONLY string from the canvas.
			theStringList.trimToSize();
			if(theStringList.isEmpty())
				undoMenuItem.setEnabled(false);
			canvas.repaint();
		}
